...
#--------------文件信息-------------------------------------------------------------------------------
#
# 文   件   名: l4matrix-bsp.mk (bsp.mk)
#
# 创   建   人: Wang.Dongfang(王东方)
#
# 文件创建日期: 2018 年 03 月 30 日
#
# 描        述: l4SylixOS BSP 目标 makefile 模板.
#*********************************************************************************************************
...
# Link script files
#*********************************************************************************************************
LOCAL_LD_SCRIPT_NT := $(LOCAL_LD_SCRIPT)
...
$($(target)_IMG): $($(target)_OBJS) $($(target)_DEPEND_TARGET)
		@rm -f $@
		$(__PRE_LINK_CMD)
		$(LD) $(__CPUFLAGS) $(ARCH_KERNEL_LDFLAGS) --abi=eabi -z --dynamic --trampolines=off --dsbt_size=64 -o $@ $(LOCAL_LD_SCRIPT) $(__OBJS) $(__LIBRARIES) 
		@mv $@ $@.c6x
		@nm $@.c6x > $@_nm.txt
