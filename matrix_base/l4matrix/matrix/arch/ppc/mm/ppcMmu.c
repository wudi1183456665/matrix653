...
typedef ULONG       MX_OBJECT_HANDLE;                                   /*  这里的对象句柄和 ID 号等同  */

static MX_OBJECT_HANDLE     _G_hPGDPartition;                           /*  系统目前仅使用一个 PGD      */
static MX_OBJECT_HANDLE     _G_hPTEPartition;                           /*  PTE 缓冲区                  */
static PMX_MMU_CONTEXT      _G_pMmuContext[MX_CFG_MAX_PROCESSORS];      /*  上下文                      */
...
#define MX_CFG_MMU_ZONE_NUM                   8                         /*  物理分区数                  */
#define MX_CFG_MMU_VIR_NUM                    8                         /*  虚拟分区数                  */
...
static INT  ppcMmuFlags2Attr (ULONG  ulFlag, UINT8  *pucPP, UINT8  *pucWIMG,  UINT8  *pucExec)
{
    ...
    if (!(ulFlag & MX_MMU_FLAG_VALID)) {                                /*  无效的映射关系              */
        return  (PX_ERROR);
    }

    if (ulFlag & MX_MMU_FLAG_ACCESS) {                                  /*  是否拥有访问权限            */
        if (ulFlag & MX_MMU_FLAG_WRITABLE) {                            /*  是否可写                    */
            *pucPP = PP_RW;
    ...
    if ((ulFlag & MX_MMU_FLAG_CACHEABLE) &&
        (ulFlag & MX_MMU_FLAG_BUFFERABLE)) {                            /*  CACHE 与 BUFFER 控制        */
...
{
    if (MX_CPU_GET_CUR_ID() == 0) {                                     /*  仅 Core0 复位 Cache         */
...
static MX_PGD_TRANSENTRY *ppcMmuPgdOffset (PMX_MMU_CONTEXT  pmmuctx, addr_t  ulAddr)
{
    ...
    p_pgdentry = (MX_PGD_TRANSENTRY *)((addr_t)p_pgdentry
               | ((ulAddr >> MX_CFG_MMU_PGD_SHIFT) << 2));              /*  获得一级页表描述符地址      */
...
{
    return  ((MX_PMD_TRANSENTRY *)p_pgdentry);                          /*  无 PMD 项                   */
}
/*********************************************************************************************************
...
{
    *pulPhysicalAddr = (addr_t)(pteentry.PTE_uiRPN << MX_CFG_MMU_PAGE_SHIFT);   /*  获得物理地址        */
