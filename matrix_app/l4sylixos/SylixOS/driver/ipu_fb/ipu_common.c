                       ...
                       ips_dev_panel_t * panel)
{
    ...
    if(csi_dma_band_mode) // enable band mode
    {
        ...
        if(idmac_info.channel == 0)
            ...
            printk("CSI channel not supported! you need to modify the interrupt registration!\n");
...
void ipu1_ch0_eobnd_isr(void)
{
    ...
    if(i%band_num != band_num - 1)
        memcpy((void *)(CH27_EBA0 + frame_width*band_height*2*(i%band_num)),
               (void *) CH23_EBA0, frame_width*band_height*2);
    else
        memcpy((void *)(CH27_EBA0 + frame_width * band_height * 2 * (i % band_num)),
               (void *) CH23_EBA0, frame_width * (cap_height - band_height * (band_num - 1)) * 2);
