...
** 输  入  : uiPinGroup  GPIO 的分组
**           uiPinNum    GPIO 在本组内的编号 0～31
**           uiCfg       管脚功能设置
** 输  出  : 0: 正确 -1:错误
** 全局变量:
** 调用模块:
**           12.15 日 修改了GpioPinmuxReg[uiPinGroup - 1][uiPinNum]为GpioPinmuxReg[uiPinGroup][uiPinNum]
**           为了符合整个gpio 的寄存器组定义规范。测试输入始终正常，根据手册是因为输入始终是连接到gpio的
**           因此那些测试输入管脚的成功是巧合。
*********************************************************************************************************/
